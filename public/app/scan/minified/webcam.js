      ...
      link: function postLink($scope, element) {
        ...
        var startWebcam = function startWebcam() {
          ...
          .then(function (devices) {
            var deviceCameras = devices.filter(function (device) {
              return device.kind === 'videoinput';
            });
            if (deviceCameras && deviceCameras.length == 1) {
              var mediaConstraint = { video: { facingMode: 'environment', optional: [{ sourceId: deviceCameras[0].deviceId }] }, audio: false };
              if (window.hasModernUserMedia) {
            ...
            } else if (deviceCameras && deviceCameras.length > 1) {
              var constraints = {
                video: {
                  optional: [{ sourceId: deviceCameras[deviceCameras.length-1].deviceId }]
                },
              };
              if (window.hasModernUserMedia) {
                // The spec has changed towards a Promise based interface
